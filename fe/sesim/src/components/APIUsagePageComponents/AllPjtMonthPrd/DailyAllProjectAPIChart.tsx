...
import { DailyProjectRequest, ProjectRequest } from "../../../types/APIUsageTypes";
...
const processDailyData = (data: DailyProjectRequest[] | null) => {
  ...
  data.forEach(dailyItem => {
    ...
    try {
      ...
      if (!dailyData[dateKey]) {
        dailyData[dateKey] = {
          date: displayDate,
          totalRequests: dailyItem.totalRequests || 0
        };
      }

      if (dailyItem.projectRequests && Array.isArray(dailyItem.projectRequests)) {
        dailyItem.projectRequests.forEach((projectRequest: ProjectRequest) => {
          dailyData[dateKey][projectRequest.projectId] = projectRequest.requestCount;
        });
      }
  ...
  return (
    <div className="flex flex-row flex-wrap gap-x-8 gap-y-2 justify-center items-center">
      <div className="flex flex-row gap-2 items-center">
        ...
        />
        <p className="text-sm font-semibold text-gray-400">총 API 호출 수</p>
      </div>
...
const extractUniqueProjects = (data: DailyProjectRequest[] | null) => {
  ...
  data.forEach(dailyData => {
    if (dailyData.projectRequests && Array.isArray(dailyData.projectRequests)) {
      dailyData.projectRequests.forEach((projectRequest: ProjectRequest) => {
        projectsMap.set(projectRequest.projectId, true);
      });
    }
  });
...
export default function DailyAllProjectAPIChart({ data }: { data: DailyProjectRequest[] | null }) {
