...
import { MonthModelRequest } from '../../../types/APIUsageTypes';
...
const transformData = (data: MonthModelRequest[] | null): ChartDataItem[] => {
  ...
  recentMonths.forEach(month => {
    month.modelRequests.forEach((model: ModelRequest) => {
  ...
  return recentMonths.map(month => {
    ...
    month.modelRequests.forEach((model: ModelRequest) => {
      result[model.modelId.toString()] = model.requestCount;
    });
...
const getMaxValue = (data: Record<string, number | string>[] | null): number => {
  if (!data || data.length === 0) {
    return 0;
  }
...
export default function MonthlyTotalAPIChartPerModels({ data }: { data: MonthModelRequest[] | null }) {
