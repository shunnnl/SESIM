...
import { DailyModelRequest } from "../../../types/APIUsageTypes";
...
const processModelDailyData = (data: DailyModelRequest[] | null): DailyModelDataItem[] => {
  ...
  data.forEach(dailyItem => {
    ...
    try {
      ...
      if (dailyItem.modelRequests && Array.isArray(dailyItem.modelRequests)) {
        dailyItem.modelRequests.forEach((modelRequest) => {
          dailyData[dateKey][modelRequest.modelId.toString()] = modelRequest.requestCount;
        });
      }
    } catch (error) {
...
export default function DailyModelRequestsChart({ data }: { data: DailyModelRequest[] | null }) {
