...
import { AllProjectsAllPeriodsData } from "../../store/APIUsageSlice";

interface AllProjectsAllPeriodsViewProps {
  data: AllProjectsAllPeriodsData | null;
}

export const AllProjectsAllPeriodsView: React.FC<AllProjectsAllPeriodsViewProps> = ({ data }) => {
  return (
      ...
      <div className="flex flex-col my-[16px] gap-4">
        <div className="flex flex-row gap-4 w-full">
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <p className="text-2xl font-bold text-white">{data?.projectCount} <span className="text-sm text-medium text-gray-400">개</span></p>
            </div>
            <div className="h-full justify-start">
        ...
        >
          <div className="flex flex-row gap-3 items-center">
            ...
            <p className="text-md font-semibold text-left text-[#DEDEDE]">최근 3개월 동안 비용 일일 그래프</p>
          </div>
          <DailyCostChart data={data?.allCostForThreeMonthsCumulative ?? []} />
        </motion.div>
      </div>
    </div>
  );
